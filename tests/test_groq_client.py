"""Tests for the Groq LLM client module."""
...
@pytest.fixture
def mock_groq():
    """Patch the Groq SDK constructor and return the mock client instance."""
    with patch("app.groq_client.Groq") as mock_cls:
...
def test_default_parameters():
    ...
    with patch("app.groq_client.Groq"):
...
def test_system_prompt_prepended(mock_groq):
    """The system prompt is always the first message sent to the API."""
    mock_groq.chat.completions.create.return_value = _make_completion("hi")
...
def test_system_prompt_with_empty_history(mock_groq):
    ...
    sent_messages = mock_groq.chat.completions.create.call_args.kwargs["messages"]
...
def test_chat_returns_assistant_content(mock_groq):
    """chat() returns the content string from the first choice."""
    mock_groq.chat.completions.create.return_value = _make_completion(
...
def test_chat_passes_model_and_params(mock_groq):
    """chat() forwards model, max_tokens, and temperature to the API."""
    mock_groq.chat.completions.create.return_value = _make_completion("ok")
    client = GroqClient(api_key="key", model="m", max_tokens=100, temperature=0.5)
...
def test_rate_limit_error_returns_fallback(mock_groq):
    """RateLimitError produces the rate-limit fallback message."""
...
def test_api_timeout_error_returns_fallback(mock_groq):
    """APITimeoutError produces the general fallback message."""
    mock_groq.chat.completions.create.side_effect = APITimeoutError(
        request=MagicMock(),
    )
    client = GroqClient(api_key="key")
...
def test_api_error_returns_fallback(mock_groq):
    """APIError produces the general fallback message."""
    mock_groq.chat.completions.create.side_effect = APIError(
...
def test_conversation_history_forwarded(mock_groq):
    """User and assistant messages are forwarded after the system prompt."""
    mock_groq.chat.completions.create.return_value = _make_completion("reply")
    client = GroqClient(api_key="key")
    ...
    sent = mock_groq.chat.completions.create.call_args.kwargs["messages"]
...
# Feature: library-ai-chatbot, Property 13: System prompt always included in LLM calls
...
def test_property_system_prompt_always_included(messages):
    ...
    with patch("app.groq_client.Groq") as mock_cls:
        ...
        # System message is present and is the very first entry
        assert len(sent_messages) >= 1
        assert sent_messages[0]["role"] == "system"
        assert sent_messages[0]["content"] == SYSTEM_PROMPT

        # The rest of the messages match the input history exactly
        assert sent_messages[1:] == messages


# Feature: library-ai-chatbot, Property 14: Token limit always set in LLM calls
@given(
...
def test_property_token_limit_always_set(messages, max_tokens):
    """For any call to the Groq client, max_tokens is always a positive
    integer in the API request."""
    with patch("app.groq_client.Groq") as mock_cls:
        mock_instance = MagicMock()
        mock_cls.return_value = mock_instance
        mock_instance.chat.completions.create.return_value = _make_completion("ok")

        client = GroqClient(api_key="key", max_tokens=max_tokens)
        client.chat(messages)
